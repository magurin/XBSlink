...
namespace XBSLink.UDP
{
    public class xbs_udp_message
    {
    ...
    public class xbs_udp_listener_statistics
    {
   ...
   public class xbs_udp_listener
    {
        ...
        private Queue<xbs_udp_message> in_audio_msgs_high_prio = new Queue<xbs_udp_message>();
        private Queue<xbs_node_message> out_audio_msgs_high_prio = new Queue<xbs_node_message>();
        ...
        public xbs_sniffer sniffer
        {
            get
            {
                return Engine.sniffer;
            }
        }
        ...
        public xbs_cloudlist cloud
        {
            get
            {
                return Engine.cloudlist;
            }
        }
 ...
 dispatcher()
            ...
            //m_pUdpServer.Bindings = new IPEndPoint[] { (IPEndPoint)local_endpoint };
...
//            if (!exiting && e.Data.Length > 0)
//            {
//                xbs_node_message_type command = xbs_node_message.getMessageTypeFromUDPPacket(e.Data);
//                xbs_udp_message msg = new xbs_udp_message();
//                msg.msg_type = command;
//                if (e.Data.Length > 3)
//                {
//                    msg.data_len = xbs_node_message.getDataLengthFromUDPPacket(e.Data);
//                    if (msg.data_len + sizeof(UInt16) + sizeof(xbs_node_message_type) == e.Data.Length)
//                    {
//                        msg.data = new byte[msg.data_len];
//                        Buffer.BlockCopy(e.Data, xbs_udp_message.HEADER_LENGTH, msg.data, 0, msg.data_len);
//                    }
//                    else
//                    {
//#if DEBUG
//                        xbs_messages.addInfoMessage("received packet with wrong length! expected " + msg.data_len + " but got " + (e.Data.Length - sizeof(UInt16) - sizeof(xbs_node_message_type)) + " bytes.", xbs_message_sender.UDP_LISTENER, xbs_message_type.ERROR);
//#endif
//                        msg = null;
//                    }
//                }
...
//                        else if (command == xbs_node_message_type.AUDIO_DATA)
//                            lock (in_audio_msgs_high_prio)
//                                in_audio_msgs_high_prio.Enqueue(msg);
//                        else
//                            lock (in_msgs)
//                                in_msgs.Enqueue(msg);
...
# endif
            switch (udp_msg.msg_type)
            {
                ...
                //case xbs_node_message_type.AUDIO_ANOUNCE:
                //    break;

                //case xbs_node_message_type.AUDIO_ACCEPT:
                ...
                case xbs_node_message_type.ANNOUNCE:
                    ...
                    if (msg_announce.hasOption(xbs_node_message_announce.OPTION_CLOUDNAME))
                        if (!msg_announce.getOption(xbs_node_message_announce.OPTION_CLOUDNAME).Equals(cloud.current_cloudname))
                            send_packet = false;
                    if (send_packet)
...
#endif
                        node_list.tryAddingNode(tmp_node, cloud.current_cloudname);
                        }
#if DEBUG
                        else
...
# endif
                ...
                case xbs_node_message_type.GETCLIENTVERSION:
                    tmp_node = (sending_node != null) ? sending_node : new xbs_node(udp_msg.src_ip, udp_msg.src_port);
                    xbs_node_message_clientversion msg_gcv = new xbs_node_message_clientversion(tmp_node, xbs_constants.xbslink_version);
                ...
                case xbs_node_message_type.NICKNAME:
                    //EN EL MOMENTO QUE ENVIA EL NICKNAME ESTÁ ON
                    if (sending_node != null)
                    {
...
#endif
            sniffer.injectRemotePacket(ref udp_msg.data, dstMAC, srcMAC, ref sending_node);
        }
...
# endif
                ...
                try
                {
                    //m_pUdpServer.SendPacket(bytes, 0, bytes.Length,(IPEndPoint) ep);
        ...
        #region KAI IN COMMANDS

        public void XBS_SendChat(xlink_msg udp_msg, string user_name, string Text)
        {
            //Engine.XlinkServer.xLinkMsgProcess.SendChatMessage(udp_msg,user_name, Text);
        }

        public void XBS_SendPM(xlink_msg udp_msg, string user_name, string Text)
        {
            //Engine.XlinkServer.xLinkMsgProcess.SendPMMessage(udp_msg,user_name, Text);
        }

        public void XBS_ChannelCreate(xlink_msg udp_msg, string CloudName, int Players, bool isPrivate, int MaxPlayers)
        {
            //Engine.XlinkServer.xLinkMsgProcess.SendCreateCloud(udp_msg,CloudName, Players, isPrivate, MaxPlayers);
        }

        public void XBS_Detach(xlink_msg udp_msg)
        {
            //KAI_CLIENT_DETACH
            //Engine.XlinkServer.xLinkMsgProcess.SendDetach(udp_msg);
        }
        ...
        public void XBS_LeaveUser(xlink_msg udp_msg, string username)
        {
            ...
            //Engine.XlinkServer.xLinkMsgProcess.LeaveUserFromVector(udp_msg,username);
        }

        public void XBS_JoinUser(xlink_msg udp_msg, string username)
        {
            //Engine.XlinkServer.xLinkMsgProcess.JoinUserToVector(udp_msg,username);
