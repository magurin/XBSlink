...
namespace XBSLink.Node.List
{
   public class xbs_node_list
    {
        ...
        public delegate void DeleteNodesHandler(List<xbs_node> nodes);
        public event DeleteNodesHandler DeleteNodes;
        ...
        public xbs_node_list()
        {
            ...
            ping_thread = new Thread(() => ping_nodes_thread());
        ...
        public List<xbs_node> CloneToList()
        {
            lock (node_list)
                return new List<xbs_node>(node_list);
        }

        public xbs_node[] CloneToArray()
        {
            lock (node_list)
        ...
        public void delNode(xbs_node node)
        {
            ...
            if (node.get_xbox_count() > 0)
                foreach (xbs_xbox xbox in node.getXboxArray())
                    nat.informOfRemovedDevice(xbox.mac);
        }
