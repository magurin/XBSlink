...
namespace XBSLink.Chat
{
    public class xbs_chat
    {
        ...
        public static xbs_node_list node
        {
            get
            {
        ...
        {
            if (node != null)
                node.sendChatMessageToAllNodes(message);
        }

        public static void addLocalMessage(String message)
        {
            if (node != null)
                addMessage("<" + node.local_node.nickname + "> : " + message);
        }
